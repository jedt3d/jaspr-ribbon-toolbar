...
IconAsset _svg(String name) {
  ...
  return IconAsset(
    ...
    data: 'data:image/svg+xml;base64,${base64Encode(bytes)}',
  );
}

Map<String, IconAsset> _icons(List<String> names) => {
  for (final n in names) n: _svg(n),
};

// ── Shared building blocks ──────────────────────────────────────────────────

RibbonTab _homeTab({
  String copyCaption = 'Copy',
  String copyTag = 'clipboard.copy',
}) {
  return RibbonTab(
    caption: 'Home',
    groups: [
